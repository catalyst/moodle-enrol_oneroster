...
use enrol_oneroster\tests\local\v1p2\csv_test_helper_v1p2;
...
class process_csv_file_check_test extends file_check_test_version_one {
    ...
    protected function setUp(): void {
        ...
        // Ensure testdir is available for v1p2 tests
        if (!isset($this->testdir)) {
            $this->testdir = make_temp_directory('csvtest_dir_v1p2');
            csv_test_helper_v1p2::setupcsvfiles($this->testdir);
        }
    }
     ...
     * @skip CSV validation needs debugging - v1p2 header validation issue
     */
    public function test_validate_csv_headers_valid_headers() {
        $this->markTestSkipped('CSV validation needs debugging - v1p2 header validation issue');
        // Test Users headers.
        $filepath = $this->testdir . DIRECTORY_SEPARATOR . 'users.csv';
        $result = \enrol_oneroster\local\v1p2\csv_client_helper::validate_csv_headers($filepath);
